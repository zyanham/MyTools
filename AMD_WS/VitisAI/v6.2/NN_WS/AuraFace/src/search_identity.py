...
def embed(session: ort.InferenceSession, image_path: Path, img_size: int, rgb: bool, run_times: List[float]) -> np.ndarray:
...
def main() -> None:
    ...
    query_embedding = embed(session, query, args.img_size, args.rgb, run_times)

    rows = []
    for image_path in gallery_images:
        candidate_embedding = embed(session, image_path, args.img_size, args.rgb, run_times)
