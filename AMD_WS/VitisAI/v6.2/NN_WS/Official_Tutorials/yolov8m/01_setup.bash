...
if [[ "${SKIP_PIP:-0}" == "1" ]]; then
    mkdir -p "$LOG_DIR"
    echo "SKIP_PIP=1; setup skipped." | tee "$LOG_DIR/setup.log"
elif [[ -f requirements.txt ]]; then
    run_logged "$LOG_DIR/setup.log" "$PYTHON_BIN" -m pip install -r requirements.txt
else
    mkdir -p "$LOG_DIR"
    echo "No requirements.txt found; setup skipped." | tee "$LOG_DIR/setup.log"
fi
