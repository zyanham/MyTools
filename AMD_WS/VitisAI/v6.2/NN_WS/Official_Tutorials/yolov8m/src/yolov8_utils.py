...
def plot_images(
    ...
    annotator = Annotator(
        mosaic, line_width=1, font_size=fs, pil=True, example=names
    )
    for i in range(i + 1):
        x, y = int(w * (i // ns)), int(h * (i % ns))  # block origin
        annotator.rectangle(
            [x, y, x + w, y + h], None, (255, 255, 255), width=1 # width was 2
        )  # borders
        if paths:
            annotator.text(
                # filenames
        ...
        if len(cls) > 0:
            ...
            for j, box in enumerate(boxes.T.tolist()):
                ...
                color = colors(c)
                names2 = names.split('\n')
                c = names2[c] if names2 else c
                if labels or conf[j] > 0.25:  # 0.25 conf thresh
                    label = f"{c}" if labels else f"{c} {conf[j]:.1f}"
                    annotator.box_label(box, label, color=color)
    #annotator.im.save(fname) # save
    # Convert PIL Image to OpenCV format if pil=True
    ...
    resized_image = cv2.resize(annotated_image, (1225, 900))

    #cv2.imshow("Annotated Image", resized_image)
    #cv2.waitKey(1)
    #cv2.destroyAllWindows()
    #display(annotator.im)
    #cv2.imwrite('inference_xx.jpg', resized_image)
    ##time.sleep(0.2)
