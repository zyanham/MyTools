...
RUN_LOG="${RUN_LOG:-$LOG_DIR/npu_run_resnet50_quark.log}"

ensure_model_file "models/resnet50-v1-12_quantized.onnx" "Run bash 02_quark.bash first."
mkdir -p input
if ! compgen -G "input/*.npy" > /dev/null; then
    "$PYTHON_BIN" -c "import numpy as np, os; os.makedirs('input', exist_ok=True); np.save('input/input0.npy', np.random.rand(1, 3, 224, 224).astype('float32'))"
fi

run_logged "$RUN_LOG" "$PYTHON_BIN" src/runmodel.py
