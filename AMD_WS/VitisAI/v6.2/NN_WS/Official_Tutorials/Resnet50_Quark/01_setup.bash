...
MODEL_URL="${MODEL_URL:-https://huggingface.co/onnxmodelzoo/resnet50-v1-12/resolve/main/resnet50-v1-12.onnx}"
CALIB_URL="${CALIB_URL:-https://github.com/microsoft/onnxruntime-inference-examples/blob/main/quantization/image_classification/cpu/test_images/daisy.jpg?raw=true}"
ensure_official_venv
mkdir -p "$LOG_DIR"
log_python_context | tee "$LOG_DIR/setup_venv.log"

if [[ -f models/resnet50-v1-12.onnx && -f calib_data/daisy.jpg ]]; then
    mkdir -p "$LOG_DIR"
    echo "Model and calibration image already exist; setup skipped." | tee "$LOG_DIR/setup.log"
else
    mkdir -p models calib_data val_data
    if [[ ! -f models/resnet50-v1-12.onnx ]]; then
        run_logged "$LOG_DIR/setup_model.log" download_file "$MODEL_URL" models/resnet50-v1-12.onnx
    fi
    if [[ ! -f calib_data/daisy.jpg ]]; then
        run_logged "$LOG_DIR/setup_calib.log" download_file "$CALIB_URL" calib_data/daisy.jpg
    fi
    if [[ ! -f val_data/daisy.jpg ]]; then
        cp calib_data/daisy.jpg val_data/daisy.jpg
    fi
    cat "$LOG_DIR"/setup_*.log > "$LOG_DIR/setup.log"
fi
