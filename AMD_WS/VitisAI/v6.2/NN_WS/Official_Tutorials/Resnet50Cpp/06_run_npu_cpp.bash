...
RUN_LOG="${RUN_LOG:-$LOG_DIR/npu_run_resnet50cpp_cpp.log}"
COMPILED_MODEL="${COMPILED_MODEL:-vek385_cache_dir/resnet50-v1-12/resnet50-v1-12.onnx}"

ensure_model_file "$COMPILED_MODEL" "Run bash 03_compile.bash first."
if [[ ! -x model-app.elf ]]; then
    echo "model-app.elf is missing or not executable; run bash 04_build_cpp_app.bash first." >&2
    exit 1
fi
...
set +e
{
    ...
    echo "Started: $(date -Is)"
    echo "Working directory: $PWD"
    echo "Command: $PYTHON_BIN src/runhelper.py"
    "$PYTHON_BIN" src/runhelper.py
    helper_status=$?
    if [[ "$helper_status" -ne 0 ]]; then
        echo "src/runhelper.py failed with status $helper_status"
        exit "$helper_status"
    fi
    start_s="$(date +%s)"
    echo "Command: ./model-app.elf $COMPILED_MODEL input0.bin output0.bin"
    ./model-app.elf "$COMPILED_MODEL" input0.bin output0.bin
    app_status=$?
    end_s="$(date +%s)"
    echo "[PERF] cpp_app_total_s=$((end_s - start_s)) status=$app_status"
    echo "Finished: $(date -Is)"
    exit "$app_status"
} 2>&1 | tee "$RUN_LOG"
status="${PIPESTATUS[0]}"
