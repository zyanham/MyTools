...
The default flow exports FP32 ONNX and compiles it for BF16 device execution.
An optional Quark VINT8 flow is also provided for INT8 NPU experiments.
...
## Optional Quark INT8 Flow

Prepare calibration images under `calib_data/`, or point `CALIB_DIR` at another
image directory. Then run:

```bash
VARIANT=s CALIB_DIR=../Dataset/Pixabay bash 07_quark_int8.bash
VARIANT=s bash 08_compile_int8.bash
VARIANT=s bash 09_run_npu_int8.bash ../Dataset/Pixabay results/npu_yolov8s_int8
```

For YOLO8m, use `VARIANT=m`.

Outputs and cache keys:

- `models/yolov8s_vint8.onnx`, cache key `yolov8s_vint8`
- `models/yolov8m_vint8.onnx`, cache key `yolov8m_vint8`

AI Analyzer:

```bash
VARIANT=s bash 10_run_npu_int8_ai_analyzer.bash ../Dataset/Pixabay results/npu_yolov8s_int8_analyzer
```
