...
　「C++デスクトップ開発」「MSVC v143 - VS 2022 C++ x64/x86 build tools」「Windows11 SDK」のインストールを実施する
  ...
  ryzen-ai-lt-1.8.0.exeをダウンロードする際、AMDアカウントが必要
  問題なければパスはデフォルトでOKC:\Program Files\RyzenAI\1.8.0\
  
  インストール後、Anacondaの環境リストにryzen-ai-1.8.0が追加されているので、仮想環境で下記を実行
  
#### Miniforge起動  
  > conda activate ryzen-ai-1.8.0  
  > cd Program Files\RyzenAI\1.8.1\quicktest  
  > python quicktest.py  
...
> git clone https://github.com/amd/RyzenAI-SW.git -b v1.8.0
