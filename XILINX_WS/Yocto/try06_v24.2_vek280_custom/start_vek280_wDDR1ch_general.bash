...
bitbake-layers create-layer ../sources/meta-wDDR1ch_general # Create a new layer for your custom hardware configuration to live (name as applicable)
bitbake-layers add-layer ../sources/meta-wDDR1ch_general # Allow bitbake to find the layer when building
mkdir ../sources/meta-wDDR1ch_general/conf/machine # Make a new folder in your layer to store machine configurations

cat << EOF > ../sources/meta-wDDR1ch_general/conf/machine/wDDR1ch_general-vek280-versal.conf 
...
sed -i "/MACHINE ??=/c\MACHINE ??= \"wDDR1ch_general-vek280-versal\"" conf/local.conf # Set the machine in conf/local.conf (change name as applicable)

# SD Card Image
echo 'IMAGE_FSTYPES += "wic"' >>  ../sources/meta-wDDR1ch_general/conf/machine/wDDR1ch_general-vek280-versal.conf # Change path to match your machine layer
