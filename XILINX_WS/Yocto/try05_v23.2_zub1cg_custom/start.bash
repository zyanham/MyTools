...
#   > cd xilinx-zub1cg-2023.1 #PetaLinux project directory
############################################################################
#cd myproject
...
cat << EOF > ../sources/meta-myhardware/conf/machine/myhardware-zub1cg-zynqmp.conf 
#Base this machine configuration off of the zub1cg board and then make changes below
require conf/machine/zynqmp-generic.conf
HDF_BASE = "file://"
# Replace with the path to your XSA file from hardware
HDF_PATH = "$ROOT/v23.2_zub1cg_custom/system_wrapper.xsa"
EOF

# Part 2 - Edit the configuration to use the newly created machine configuration
sed -i "/MACHINE ??=/c\MACHINE ??= \"myhardware-zub1cg-zynqmp\"" conf/local.conf # Set the machine in conf/local.conf (change name as applicable)

# SD Card Image
echo 'IMAGE_FSTYPES += "wic"' >>  ../sources/meta-myhardware/conf/machine/myhardware-zub1cg-zynqmp.conf # Change path to match your machine layer
